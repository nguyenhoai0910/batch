...
IF %_status%==true (
	SYNC
	pushd \\10.102.20.10\d$\Website
	ECHO website-public\dist\ocb-fe\browser\Uploads \\10.102.20.10\d$\Website\FileBuildBackEnd\Resources
	REM FOR %%x IN (website-public\dist\ocb-fe\browser\Uploads \\10.102.20.10\d$\Website\FileBuildBackEnd\Resources) DO (ECHO %%x && xcopy %%x C:\OMIT\%%x /D /E /H /C /I /Y /F ) >> %_log%
	popd \\10.102.20.10\d$\Website\website-public\dist\ocb-fe\browser\Uploads
)	
type %_log%
ECHO ====================================================================================
ECHO Completed !
